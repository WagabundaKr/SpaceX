...
spacex_df = pd.read_csv('/Users/iryna/python/DA/full_project/spacex_launch_dash.csv')
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(value_counts, values='count', 
                    names='class', 
...
def get_scatter_plot(entered_site, payload_range):
    # Фильтруем по диапазону payload:
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= payload_range[0]) &
                            (spacex_df['Payload Mass (kg)'] <= payload_range[1])
                            ]
    # Фильтруем по выбранному Launch Site:
    if entered_site != 'ALL':
